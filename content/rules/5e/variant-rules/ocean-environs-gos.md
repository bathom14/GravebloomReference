...
|-----------|-----------------------|
| 1 | `dice:2d10\|noform\|noparens\|avg` (`2d10`) [sahuagin](compendium/5e/bestiary/fiend/sahuagin-warrior.md) |
| 2 | 1 [giant octopus](compendium/5e/bestiary/beast/giant-octopus.md) |
| 3 | `dice:1d4+1\|noform\|noparens\|avg` (`1d4 + 1`) [chuuls](compendium/5e/bestiary/aberration/chuul.md) |
| 4 | 1 [sea hag](compendium/5e/bestiary/fey/sea-hag.md) and `dice:2d4\|noform\|noparens\|avg` (`2d4`) [merrow](compendium/5e/bestiary/monstrosity/merrow.md) |
| 5 | 1 [plesiosaurus](compendium/5e/bestiary/beast/plesiosaurus.md) |
| 6 | `dice:1d4\|noform\|noparens\|avg` (`d4`) [swarms of quippers](compendium/5e/bestiary/beast/swarm-of-piranhas.md) |
| 7 | `dice:2d4\|noform\|noparens\|avg` (`2d4`) [reef sharks](compendium/5e/bestiary/beast/reef-shark.md) |
| 8 | 1 [giant shark](compendium/5e/bestiary/beast/giant-shark.md) |
| 9 | `dice:1d6\|noform\|noparens\|avg` (`d6`) items from "Magic Item Table A" in chapter 7, ""Treasure"," of the "Dungeon Master's Guide" |
...
Uneven and sharp, coral reefs can harm those forced into them. A creature forcibly moved on the reef—without using the creature's action, bonus action, or reaction—takes `dice:1d6|noform|noparens|avg|text(3)` (`d6`) slashing damage per 5 feet of reef that it is pushed over.

## Currents

Ocean water is moved by wind and tides to create constant river-like flows called currents. Tidal currents are typically the strongest. They're usually found within 50 miles of shore and at depths of less than 300 feet. These currents have a speed of `dice:1d6|noform|noparens|avg` (`d6`) miles per hour that changes daily.

Currents become weaker farther out to sea and deeper in the ocean. These currents have a speed of `dice:1d4|noform|noparens|avg` (`d4`)−1 miles per hour that changes daily.
...
Ghost fog contains the souls of murder victims whose bodies were thrown into the sea. A DC 15 Intelligence ([Arcana](rules/5e/skills.md#Arcana)) check reveals that this mist is ghost fog.

The souls of the dead want the living to join them. Every hour a ship is immersed in ghost fog, roll any die. On an even number, `dice:2d4|noform|noparens|avg` (`2d4`) hostile [specters](compendium/5e/bestiary/undead/specter.md) appear and attack those on the ship. If a humanoid creature dies in ghost fog, its spirit rises as a specter that is hostile toward all creatures that aren't undead.

All non-undead creatures immersed in ghost fog are vulnerable to necrotic damage.

### Shadowfell Fog

Shadowfell fog seeps into the Material Plane at thin points in the boundary between the two planes. A DC 15 Intelligence ([Arcana](rules/5e/skills.md#Arcana)) check reveals that this mist is Shadowfell fog.
...
Few know the origins of wild magic fog, with some claiming it is the creation of capricious gods while others believe it is the result of magical experimentation gone wrong. A DC 15 Intelligence ([Arcana](rules/5e/skills.md#Arcana)) check reveals that this mist is wild magic fog.

Whenever a creature in wild magic fog casts a spell of 1st level or higher, roll on the [Wild Magic Surge](compendium/5e/tables/wild-magic-surge-phb.md) table in chapter 3, ""Classes"," of the "Player's Handbook" to create a magical effect.
...
When a creature moves within 30 feet of a kraken's grave or starts its turn there, that creature must succeed on a DC 14 Dexterity ([Stealth](rules/5e/skills.md#Stealth)) check or it disturbs the grave. If a creature disturbs the grave, a tendril made of lightning stretches from the ground and attacks the creature with a +7 bonus to hit. On a hit, the target takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) lightning damage, and it must succeed on a DC 14 Constitution saving throw or become [blinded](rules/5e/conditions.md#Blinded) for the next `dice:2d4|noform|noparens|avg` (`2d4`) hours.
...
Any creature that can see the lure lights must succeed on a DC 14 Wisdom saving throw or be [charmed](rules/5e/conditions.md#Charmed) by the lights for 24 hours or until the lights are destroyed. A creature that succeeds on the saving throw is immune to the effect of the lure lights for 24 hours.

While [charmed](rules/5e/conditions.md#Charmed) by the lights, a creature can't willingly move out of line of sight of them and defends them to the death. If forcibly moved away from the lights, the creature tries to find its way back to them. The creature can't take a short or long rest while under this effect. After 24 hours, the [charmed](rules/5e/conditions.md#Charmed) creature gains one level of [exhaustion](rules/5e/conditions.md#Exhaustion) and must repeat the saving throw if it is within line of sight of the lights, ending the effect on itself on a success. If the lights aren't within line of sight at this time, the creature succeeds automatically.
...
When conjuration magic churns within a storm's heart, creatures of wind and rain emerge from distant realms to attack anything they encounter. The ship encounters `dice:1d3|noform|noparens|avg` (`d3`) hostile [air elementals](compendium/5e/bestiary/elemental/air-elemental.md).

### Divination

When divination magic seeps into a storm, howling winds and residual whispers temporarily scramble the knowledge of a ship's crew; for `dice:1d3|noform|noparens|avg` (`d3`) days, the ship's crew suffers disadvantage on all quality checks.
...
These ferocious storms are wracked with thunderbolts, driving sheets of acid rain, exploding meteors, and other dangers. After each day of the storm, every component aboard the ship takes `dice:1d10|noform|noparens|avg` (`d10`) fire, `dice:1d10|noform|noparens|avg` (`d10`) acid, and `dice:1d10|noform|noparens|avg` (`d10`) lightning damage. In addition, roll a `dice:d20|noform|noparens|avg` (`d20`) and add the crew's quality. On a 10 or less, the crew's quality decreases by 1, and `dice:1d6|noform|noparens|avg` (`d6`) members of the crew are killed.
...
Ghosts howl and whirl in this storm's wind, while the remains of long-dead mariners stir in their watery graves. During the storm, `dice:1d4|noform|noparens|avg` (`d4`) [specters](compendium/5e/bestiary/undead/specter.md), `dice:2d4|noform|noparens|avg` (`2d4`) [ghouls](compendium/5e/bestiary/undead/ghoul.md), and `dice:4d6|noform|noparens|avg` (`4d6`) [zombies](compendium/5e/bestiary/undead/zombie.md) emerge from the waves to attack the ship.
...
| dice: d10 | Creatures or Treasure |
|-----------|-----------------------|
| 1 | 1 [sahuagin priestess](compendium/5e/bestiary/fiend/sahuagin-priest.md) and `dice:2d10\|noform\|noparens\|avg` (`2d10`) [sahuagin](compendium/5e/bestiary/fiend/sahuagin-warrior.md) |
| 2 | 1 [young bronze dragon](compendium/5e/bestiary/dragon/young-bronze-dragon.md) |
| 3 | `dice:4d10\|noform\|noparens\|avg` (`4d10`) [giant crabs](compendium/5e/bestiary/beast/giant-crab.md) |
| 4 | `dice:3d10\|noform\|noparens\|avg` (`3d10`) [giant seahorses](compendium/5e/bestiary/beast/giant-seahorse.md) |
| 5 | `dice:1d4\|noform\|noparens\|avg` (`d4`) [hunter sharks](compendium/5e/bestiary/beast/hunter-shark.md) |
| 6 | 1 [giant octopus](compendium/5e/bestiary/beast/giant-octopus.md) |
| 7 | `dice:2d6\|noform\|noparens\|avg` (`2d6`) [merrow](compendium/5e/bestiary/monstrosity/merrow.md) |
| 8 | `dice:2d10\|noform\|noparens\|avg` (`2d10`) [merfolk](compendium/5e/bestiary/elemental/merfolk-skirmisher.md) |
| 9 | `dice:1d6\|noform\|noparens\|avg` (`d6`) items from [Magic Item Table B](compendium/5e/tables/magic-item-table-b.md) in chapter 7, ""Treasure"," of the "Dungeon Master's Guide" |
...
| First mate | Charisma ([Intimidation](rules/5e/skills.md#Intimidation)) |
| Bosun | Strength ([carpenter's tools](compendium/5e/items/carpenters-tools.md)) |
| Quartermaster | Wisdom ([Nature](rules/5e/skills.md#Nature)) |
^whirlpool-checks
...
| Failure | The vessel is immediately moved toward the vortex's center at the whirlpool's velocity, and the vessel is [restrained](rules/5e/conditions.md#Restrained) by the whirlpool until the start of its next turn. |
