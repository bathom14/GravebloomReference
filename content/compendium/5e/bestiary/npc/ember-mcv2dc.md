...
- 5e/monster/type/dragon/adult-chromatic
aliases:
- Ember
---
# Ember
...
- **Skills** [Perception](rules/5e/skills.md#Perception) +16, [Religion](rules/5e/skills.md#Religion) +10, [Stealth](rules/5e/skills.md#Stealth) +14
...
***Calescent Aura.*** At the start of Ember's turn, Ember can force any number of creatures of his choice within 10 feet of himself to make a DC 22 Constitution saving throw. On a failed save, a creature takes `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) fire damage and is [frightened](rules/5e/conditions.md#Frightened) of Ember until the start of its next turn.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+16|noform|noparens|text(+16)` to hit, reach 10 ft., one target. *Hit:* `dice:2d10+15|noform|noparens|avg|text(26)` (`2d10 + 15`) piercing damage plus `dice:2d6+6|noform|noparens|avg|text(13)` (`2d6 + 6`) fire damage.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+16|noform|noparens|text(+16)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+15|noform|noparens|avg|text(22)` (`2d6 + 15`) slashing damage.

***Firestorm Breath (Recharge 5-6).*** Ember exhales roiling flames and ash in a 90-foot cone. Each creature in that area must make a DC 22 Dexterity saving throw. On a failed save, a creature takes `dice:16d6|noform|noparens|avg|text(56)` (`16d6`) fire damage and is pushed up to 30 feet away from Ember and knocked [prone](rules/5e/conditions.md#Prone). On a successful save, a creature takes half as much damage with no other effects. If a creature is reduced to 0 hit points by this effect, the creature immediately dies, and its body is reduced to ash.
...
***Searing Bite (Costs 2 Actions).*** Ember makes one Bite attack. If the attack hits, it deals an additional `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) fire damage.
```
^statblock