...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d10+2|noform|noparens|avg|text(7)` (`1d10 + 2`) piercing damage plus `dice:1d4|noform|noparens|avg|text(2)` (`d4`) cold damage.

***Cold Breath (Recharge 5-6).*** The dragon exhales an icy blast of hail in a 15-foot cone. Each creature in that area must make a DC 12 Constitution saving throw, taking `dice:5d8|noform|noparens|avg|text(22)` (`5d8`) cold damage on a failed save, or half as much damage on a successful one.
```
^statblock