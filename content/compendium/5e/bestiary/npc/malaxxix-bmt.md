...
The [Unborn Palanquin](compendium/5e/objects/unborn-palanquin-bmt.md) is a wormlike leviathan of screeching gears made up of hundreds of tireless mezzoloths. It is a unique vehicle and Gargantuan object.
...
- **Skills** [Arcana](rules/5e/skills.md#Arcana) +12, [Athletics](rules/5e/skills.md#Athletics) +12, [Perception](rules/5e/skills.md#Perception) +11
...
***Forge Hammer.*** *Melee  or Ranged Weapon Attack:* `dice:1d20+12|noform|noparens|text(+12)` to hit, reach 10 ft. or range 60/180 ft., one target. *Hit:* `dice:4d10+6|noform|noparens|avg|text(28)` (`4d10 + 6`) bludgeoning damage, and the target can't take reactions until the end of Malaxxix's next turn. *Hit or Miss:* The hammer then magically returns to Malaxxix's hand.

***Whip.*** *Melee Weapon Attack:* `dice:1d20+12|noform|noparens|text(+12)` to hit, reach 30 ft., one creature. *Hit:* `dice:2d8+6|noform|noparens|avg|text(15)` (`2d8 + 6`) slashing damage, and the target has the [restrained](rules/5e/conditions.md#Restrained) condition for 1 minute. The [restrained](rules/5e/conditions.md#Restrained) target can make a DC 20 Strength saving throw at the end of each of its turns, ending the effect on itself on a success. Only one creature can be [restrained](rules/5e/conditions.md#Restrained) by the whip at a time.

***Mezzoloth Vortex (Recharge 5-6).*** Malaxxix summons a vortex of whirling mezzoloths at a point it can see within 90 feet of itself. The vortex is a 30-foot-radius, 100-foot-high cylinder centered on that point. Each creature other than Malaxxix in that area must make a DC 20 Dexterity saving throw. On a failed save, a creature takes `dice:10d6|noform|noparens|avg|text(35)` (`10d6`) force damage and has the [restrained](rules/5e/conditions.md#Restrained) condition while within the cylinder. On a successful save, a creature takes half as much damage and is pushed to the nearest unoccupied space outside the cylinder. The vortex lasts until the start of Malaxxix's next turn.

## Legendary Actions

***Fiendish Stride.*** Malaxxix moves up to its speed. This movement doesn't provoke opportunity attacks. When Malaxxix moves within 5 feet of a creature during this movement, that creature takes `dice:1d10|noform|noparens|avg|text(5)` (`d10`) lightning damage. A creature can take this damage only once per turn.

***Attack (Costs 2 Actions).*** Malaxxix makes one Forge Hammer or Whip attack.

![Malaxxix](compendium/5e/bestiary/legendary-group/malaxxix-bmt.md)
```
^statblock