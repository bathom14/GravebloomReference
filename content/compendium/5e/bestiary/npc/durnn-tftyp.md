...
- **Armor Class** 19 ([splint armor](compendium/5e/items/splint-armor-phb.md), [shield](compendium/5e/items/shield-phb.md))
- **Hit Points** 18 (`2d8 + 2`)
- **Speed** 30 ft.
...
***Martial Advantage.*** Once per turn, the hobgoblin can deal an extra `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) damage to a creature it hits with a weapon attack if that creature is within 5 feet of an ally of the hobgoblin that isn't [incapacitated](rules/5e/conditions.md#Incapacitated).

## Actions

***Longsword.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+1|noform|noparens|avg|text(5)` (`1d8 + 1`) slashing damage, or `dice:1d10+2|noform|noparens|avg|text(6)` (`1d10 + 2`) slashing damage if used with two hands.
