...
***Stable.*** Whenever an effect knocks the turtle [prone](rules/5e/conditions.md#Prone), it can make a DC 10 Constitution saving throw to avoid being knocked [prone](rules/5e/conditions.md#Prone). A [prone](rules/5e/conditions.md#Prone) turtle is upside down. To stand up, it must succeed on a DC 10 Dexterity check on its turn and then use all its movement for that turn.

## Actions

***Bite.*** *Melee Weapon Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 5 ft., one target. *Hit:* `dice:4d6+4|noform|noparens|avg|text(18)` (`4d6 + 4`) slashing damage.
```
^statblock