...
***Salt Osmosis.*** Whenever the snail starts its turn in contact with a pound or more of salt, it takes `dice:1d4|noform|noparens|avg` (`d4`) necrotic damage. Using an action to sprinkle a pound of salt on the snail deals `dice:1d4|noform|noparens|avg` (`d4`) necrotic damage to it immediately and another `dice:1d4|noform|noparens|avg` (`d4`) necrotic damage to it at the start of its next turn (after which the salt rubs off), provided the snail has not withdrawn into its shell.
