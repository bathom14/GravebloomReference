...
***Claws.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+2|noform|noparens|avg|text(9)` (`2d6 + 2`) slashing damage. If the grinning cat was [invisible](rules/5e/conditions.md#Invisible) before it attacked, the target must succeed on a DC 12 Strength saving throw or have the [prone](rules/5e/conditions.md#Prone) condition. If the target has the [prone](rules/5e/conditions.md#Prone) condition, the cat can make a Bite attack against it as a bonus action.

***Fade Away.*** The grinning cat magically becomes [invisible](rules/5e/conditions.md#Invisible) for 1 hour or until it attacks, gradually fading away over the course of its turn. It can choose to leave part of its body visible, such as its tail, its head, or its grinning mouth. Any equipment the cat wears or carries is [invisible](rules/5e/conditions.md#Invisible) with it.
