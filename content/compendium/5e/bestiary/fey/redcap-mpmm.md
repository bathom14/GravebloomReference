...
***Iron Boots.*** The redcap has disadvantage on Dexterity ([Stealth](rules/5e/skills.md#Stealth)) checks.
...
***Wicked Sickle.*** *Melee Weapon Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 5 ft., one target. *Hit:* `dice:2d4+4|noform|noparens|avg|text(9)` (`2d4 + 4`) slashing damage.

***Ironbound Pursuit.*** The redcap moves up to its speed to a creature it can see and kicks with its iron boots. The target must succeed on a DC 14 Dexterity saving throw or take `dice:3d10+4|noform|noparens|avg|text(20)` (`3d10 + 4`) bludgeoning damage and be knocked [prone](rules/5e/conditions.md#Prone).
```
^statblock
