...
Wisdom saving throw. On a failure the creature is [incapacitated](rules/5e/conditions.md#Incapacitated) until the start of its next turn. On a success, the creature suffers no effect.
...
***Barbed Tail.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) piercing damage.
```
^statblock