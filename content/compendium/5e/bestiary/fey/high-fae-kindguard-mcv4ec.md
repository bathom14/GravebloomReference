...
- **Skills** [Acrobatics](rules/5e/skills.md#Acrobatics) +10, [Athletics](rules/5e/skills.md#Athletics) +10, [Perception](rules/5e/skills.md#Perception) +7
...
***Evasion.*** If the high fae is subjected to an effect that allows it to make a Dexterity saving throw to take only half damage, it instead takes no damage if it succeeds on the saving throw and only half damage if it fails, provided it doesn't have the [incapacitated](rules/5e/conditions.md#Incapacitated) condition.
...
***Fae Claymore.*** *Melee Weapon Attack:* `dice:1d20+10|noform|noparens|text(+10)` to hit, reach 10 ft., one target. *Hit:* `dice:4d6+6|noform|noparens|avg|text(20)` (`4d6 + 6`) force damage, and the target's speed is reduced by 10 feet, to a minimum speed of 5 feet. The reduction lasts until the start of the high fae's next turn.

***Tripping Feint.*** The high fae targets one Large or smaller creature it can see within 10 feet of itself. The target must succeed on a DC 18 Dexterity saving throw or have the [prone](rules/5e/conditions.md#Prone) condition.
