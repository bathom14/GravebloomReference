...
- **Skills** [Arcana](rules/5e/skills.md#Arcana) +6, [Insight](rules/5e/skills.md#Insight) +5, [Sleight of Hand](rules/5e/skills.md#Sleight%20of%20Hand) +9, [Stealth](rules/5e/skills.md#Stealth) +6
...
***Spectral Card.*** *Melee  or Ranged Spell Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 5 ft. or range 60 ft., one target. *Hit:* `dice:1d10+3|noform|noparens|avg|text(8)` (`1d10 + 3`) force damage plus `dice:1d10|noform|noparens|avg|text(5)` (`d10`) damage that is radiant if the `dice:d10|noform|noparens|avg` (`d10`) roll is an even number and necrotic if it's odd.

***Card Spray (Recharge 5-6).*** The riffler magically unleashes a spray of spectral cards in a 30-foot cone. Each creature in that area must make a DC 14 Dexterity saving throw. On a failed save, a creature takes `dice:5d10|noform|noparens|avg|text(27)` (`5d10`) force damage and has the [restrained](rules/5e/conditions.md#Restrained) condition for 1 minute as cards bind it. On a successful save, a creature takes half as much damage only. A [restrained](rules/5e/conditions.md#Restrained) creature can repeat the saving throw at the end of each of its turns, ending the effect on itself on a success.

***Plane Shift.*** The riffler casts the [Plane Shift](compendium/5e/spells/plane-shift.md) spell targeting only itself, requiring no material components and using Charisma as the spellcasting ability.

## Reactions

***Shuffle Destiny.*** When a creature the riffler can see within 30 feet of itself makes an ability check, an attack roll, or a saving throw, the riffler can magically force that creature to roll a `dice:d6|noform|noparens|avg` (`d6`) and either add the number rolled to the total or subtract it from the total (riffler's choice), potentially changing the outcome.
```
^statblock