...
If you have "Bigby Presents: Glory of the Giants", you can use the [Goose Egg Trinket](compendium/5e/items/goose-egg-trinket-bgg.md) table to determine the contents of one of the Goose Mother's eggs that isn't destined to hatch into a goose or a giant goose (also described in that book).
...
- **Skills** [Insight](rules/5e/skills.md#Insight) +7, [Investigation](rules/5e/skills.md#Investigation) +4, [Perception](rules/5e/skills.md#Perception) +10
...
***Beak.*** *Melee Weapon Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 10 ft., one target. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) bludgeoning damage.

***Wing.*** *Melee Weapon Attack:* `dice:1d20+6|noform|noparens|text(+6)` to hit, reach 10 ft., one target. *Hit:* `dice:1d12+3|noform|noparens|avg|text(9)` (`1d12 + 3`) bludgeoning damage. If the target is a Large or smaller creature, it must succeed on a DC 15 Strength saving throw or have the [prone](rules/5e/conditions.md#Prone) condition.

***Lullaby of Honks (Recharge 5-6).*** The Goose Mother's heads honk a discordant lullaby. Each creature of the Goose Mother's choice within 10 feet of it must make a DC 15 Constitution saving throw. On a failed save, a creature takes `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) thunder damage and falls into a magical slumber. On a successful save, the target takes half as much damage only. The honks can be heard up to 300 feet away.

A creature under magical slumber has the [unconscious](rules/5e/conditions.md#Unconscious) condition for 8 hours. This effect ends early for a creature if it takes damage or another creature uses an action to shake it awake.
```
^statblock