...
You can use the [Goose Egg Trinket](compendium/5e/items/goose-egg-trinket-bgg.md) table to determine the contents of a golden egg. A giant goose's egg can be a great way to give characters an item that's important to the story of an adventure or your campaign.
...
***Beak.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 10 ft., one target. *Hit:* `dice:2d6+2|noform|noparens|avg|text(9)` (`2d6 + 2`) bludgeoning damage.

***Wing.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d10+2|noform|noparens|avg|text(7)` (`1d10 + 2`) bludgeoning damage, and the target must succeed on a DC 12 Strength saving throw or have the [prone](rules/5e/conditions.md#Prone) condition.

***Thunderous Honk (Recharge 5-6).*** The goose honks with ear-splitting volume. Each other creature within 30 feet of the goose must make a DC 12 Constitution saving throw. On a failed save, a creature takes `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) thunder damage and has the [deafened](rules/5e/conditions.md#Deafened) condition until the start of the goose's next turn. On a successful save, a creature takes half as much damage only. The honk can be heard within 300 feet.
```
^statblock