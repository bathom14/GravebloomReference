...
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/MPP/Transcendent%20Order%20Conduit.webp#token)
...
- **Skills** [Acrobatics](rules/5e/skills.md#Acrobatics) +7, [Perception](rules/5e/skills.md#Perception) +7, [Performance](rules/5e/skills.md#Performance) +4
...
***Unarmed Strike.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+4|noform|noparens|avg|text(11)` (`2d6 + 4`) bludgeoning damage. If the target is a creature, the conduit can choose one of the following additional effects (up to once per turn each):

- **Incapacitate.** The target must succeed on a DC 15 Constitution saving throw or have the [incapacitated](rules/5e/conditions.md#Incapacitated) condition until the end of the conduit's next turn.  
...
***Deflect Attack.*** In response to being hit by an attack roll, the conduit partially deflects the blow. The damage the conduit takes from the attack is reduced by `dice:1d10|noform|noparens|avg` (`d10`).
