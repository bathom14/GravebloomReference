...
- **Skills** [Athletics](rules/5e/skills.md#Athletics) +2, [Perception](rules/5e/skills.md#Perception) +5, [Stealth](rules/5e/skills.md#Stealth) +5
...
***Light Crossbow.*** *Ranged Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit (the target gains no benefit from less than total cover), range 320 ft., one target. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) piercing damage. *Hit or Miss:* Immediately after making this attack, the harengon can use the Hide action.
```
^statblock