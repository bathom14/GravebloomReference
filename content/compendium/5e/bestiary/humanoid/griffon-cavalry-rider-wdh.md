...
- **Armor Class** 17 ([half plate armor](compendium/5e/items/half-plate-armor-phb.md))
...
- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Animal Handling](rules/5e/skills.md#Animal%20Handling) +3, [Athletics](rules/5e/skills.md#Athletics) +4, [Perception](rules/5e/skills.md#Perception) +3
...
***Lance.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit (with disadvantage against a target within 5 ft.), reach 10 ft., one target. *Hit:* `dice:1d12+2|noform|noparens|avg|text(8)` (`1d12 + 2`) piercing damage, or `dice:1d12+5|noform|noparens|avg|text(11)` (`1d12 + 5`) piercing damage while mounted.

***Dagger.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+2|noform|noparens|avg|text(4)` (`1d4 + 2`) piercing damage. Or Ranged Weapon Attack: `dice:1d20+4|noform|noparens|text(+4)` to hit, range 20/60 ft., one target. *Hit:* `dice:1d4+2|noform|noparens|avg|text(4)` (`1d4 + 2`) piercing damage.

***Feather Fall.*** The rider wears a magic ring with which it can cast the [feather fall](compendium/5e/spells/feather-fall.md) spell on itself once as a reaction to falling. After the spell is cast, the ring becomes nonmagical.
```
^statblock