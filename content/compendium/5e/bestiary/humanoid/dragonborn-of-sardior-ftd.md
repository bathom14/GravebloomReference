...
- **Skills** [Arcana](rules/5e/skills.md#Arcana) +7, [History](rules/5e/skills.md#History) +7, [Perception](rules/5e/skills.md#Perception) +5
...
***Mind Blade.*** *Melee  or Ranged Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft. or range 20/60 ft., one target. *Hit:* `dice:1d4+4|noform|noparens|avg|text(6)` (`1d4 + 4`) piercing damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) psychic damage.

***Heat Breath (Recharge 6).*** The dragonborn exhales a wave of intense heat in a 30-foot cone. Each creature in that area must make a DC 14 Constitution saving throw, taking `dice:6d8|noform|noparens|avg|text(27)` (`6d8`) fire damage on a failed save, or half as much damage on a successful one. Metal objects in that area glow red-hot until the end of the dragonborn's next turn. Any creature in physical contact with a heated object at the start of its turn must make a DC 14 Constitution saving throw. On a failed save, the creature takes `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) fire damage and has disadvantage on attack rolls until the start of its next turn.

***Spellcasting (Psionics).*** The dragonborn casts one of the following spells, requiring no spell components and using Intelligence as the spellcasting ability (spell save DC 15, `dice:1d20+7|noform|noparens|text(+7)` to hit with spell attacks):

**1/day each:** [Bigby's hand](compendium/5e/spells/bigbys-hand.md), [hypnotic pattern](compendium/5e/spells/hypnotic-pattern.md), [telekinesis](compendium/5e/spells/telekinesis.md)
```
^statblock