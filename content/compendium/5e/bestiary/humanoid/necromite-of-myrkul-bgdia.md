...
***Skull Flail.*** *Melee Weapon Attack:* `dice:1d20+2|noform|noparens|text(+2)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8|noform|noparens|avg|text(4)` (`d8`) bludgeoning damage.

***Claws of the Grave.*** *Ranged Spell Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, range 90 ft., one target. *Hit:* `dice:2d4+3|noform|noparens|avg|text(8)` (`2d4 + 3`) necrotic damage.
```
^statblock