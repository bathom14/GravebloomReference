...
***Living Shadow.*** While in [Dim Light](rules/5e/variant-rules/dim-light-xphb.md) or [Darkness](rules/5e/variant-rules/darkness-xphb.md), the dragon has [Resistance](rules/5e/variant-rules/resistance-xphb.md) to damage that isn't Force, Psychic, or Radiant.

***Sunlight Sensitivity.*** While in sunlight, the dragon has [Disadvantage](rules/5e/variant-rules/disadvantage-xphb.md) on ability checks and attack rolls.

## Actions

***Multiattack.*** The dragon makes two Rend attacks.

***Rend.*** *Melee Attack Roll:* `dice:1d20+5|noform|noparens|text(+5)`, reach 10 ft. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) Slashing damage plus `dice:1d6|noform|noparens|avg|text(3)` (`d6`) Necrotic damage.

***Shadow Breath (Recharge 5-6).*** *Dexterity Saving Throw:* DC 13, each creature in a 30-foot [Cone](rules/5e/variant-rules/cone-area-of-effect-xphb.md). *Failure:* `dice:5d6|noform|noparens|avg|text(17)` (`5d6`) Necrotic damage. *Success:* Half damage. *Failure or Success:* A Humanoid reduced to 0 [Hit Points](rules/5e/variant-rules/hit-points-xphb.md) by this damage dies, and a [Shadow](compendium/5e/bestiary/undead/shadow.md) rises from its corpse. The shadow is under the dragon's control and shares the dragon's [Initiative](rules/5e/variant-rules/initiative-xphb.md) count but acts immediately after the dragon.

## Bonus Actions

***Shadow Stealth.*** While in [Dim Light](rules/5e/variant-rules/dim-light-xphb.md) or [Darkness](rules/5e/variant-rules/darkness-xphb.md), the dragon takes the Hide action.
```
^statblock
