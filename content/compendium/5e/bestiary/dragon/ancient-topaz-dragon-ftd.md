...
- **Skills** [Intimidation](rules/5e/skills.md#Intimidation) +17, [Perception](rules/5e/skills.md#Perception) +16, [Stealth](rules/5e/skills.md#Stealth) +7
...
***Fabricate (1/Day).*** The dragon can cast [fabricate](compendium/5e/spells/fabricate.md), requiring no spell components and using Intelligence as the spellcasting ability.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+12|noform|noparens|text(+12)` to hit, reach 15 ft., one target. *Hit:* `dice:2d10+6|noform|noparens|avg|text(17)` (`2d10 + 6`) piercing damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) necrotic damage.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+12|noform|noparens|text(+12)` to hit, reach 10 ft., one target. *Hit:* `dice:2d6+6|noform|noparens|avg|text(13)` (`2d6 + 6`) slashing damage.

***Desiccating Breath (Recharge 5-6).*** The dragon exhales yellowish necrotic energy in a 90-foot cone. Each creature in that area must make a DC 20 Constitution saving throw. On a failed save, the creature takes `dice:14d6|noform|noparens|avg|text(49)` (`14d6`) necrotic damage and is weakened until the end of its next turn. A weakened creature has disadvantage on Strength-based ability checks and Strength saving throws, and the creature's weapon attacks that rely on Strength deal half damage. On a successful save, the creature takes half as much damage and isn't weakened.
...
**1/day each:** [antilife shell](compendium/5e/spells/antilife-shell.md), [bane](compendium/5e/spells/bane.md), [control water](compendium/5e/spells/control-water.md), [create or destroy water](compendium/5e/spells/create-or-destroy-water.md)
...
***Essential Reduction (Costs 3 Actions).*** The dragon targets a creature or an object not being worn or carried that it can see within 60 feet of it. The target must succeed on a DC 19 Constitution saving throw or take `dice:9d8|noform|noparens|avg|text(40)` (`9d8`) necrotic damage. If this damage reduces the target to 0 hit points, it crumbles to dust.

![Topaz Dragon](compendium/5e/bestiary/legendary-group/topaz-dragon-ftd.md)
```
^statblock