...
***Earth Armor.*** The clockwork doesn't provoke [opportunity attacks](rules/5e/actions.md#Opportunity%20Attack) when it burrows.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) piercing damage plus `dice:1d6|noform|noparens|avg|text(3)` (`d6`) lightning damage.

***Lightning Flare (Recharges after a Short or Long Rest).*** Each creature in contact with the ground within 15 feet of the clockwork must make a DC 13 Dexterity saving throw, taking `dice:4d6|noform|noparens|avg|text(14)` (`4d6`) lightning damage on a failed save, or half as much damage on a successful one.
```
^statblock
