...
- **Skills** [Athletics](rules/5e/skills.md#Athletics) +14, [Perception](rules/5e/skills.md#Perception) +16
...
***Arcane Leak.*** When the adranach is reduced to half its hit point maximum (145 hit points), its mithral mask cracks and its arcane form begins to waver, creating a field of unstable magic around it. Any creature that starts its turn within 20 feet of the adranach or enters that area for the first time on a turn takes `dice:6d6|noform|noparens|avg|text(21)` (`6d6`) radiant damage. Additionally, if a creature casts a spell within this area, it must make a DC 18 ability check using its spellcasting ability. On a failure, the spell backfires, consuming the spell slot and dealing `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) force damage to the caster for each level of the spell slot that was consumed.
...
***Slipstream.*** The adranach can cast [invisibility](compendium/5e/spells/invisibility.md) on itself at will. While it is [invisible](rules/5e/conditions.md#Invisible), its speed is doubled.
...
***Claws.*** *Melee Weapon Attack:* `dice:1d20+14|noform|noparens|text(+14)` to hit, one target, reach 5 ft. *Hit:* `dice:4d12+7|noform|noparens|avg|text(33)` (`4d12 + 7`) slashing damage.

***Tail.*** *Melee Weapon Attack:* `dice:1d20+14|noform|noparens|text(+14)` to hit, one target, reach 15 ft. *Hit:* `dice:2d12+7|noform|noparens|avg|text(20)` (`2d12 + 7`) bludgeoning damage.

***Force Bolts (Recharge 5-6).*** The adranach targets up to six creatures it can see within 120 feet of it. Each target must succeed on a DC 22 Dexterity saving throw or take `dice:10d6|noform|noparens|avg|text(35)` (`10d6`) force damage.

## Legendary Actions

***Claw Attack.*** The adranach makes one claw attack.

***Vanish (Costs 2 Actions).*** The adranach casts [invisibility](compendium/5e/spells/invisibility.md) and moves up to its walking or flying speed. (This movement doesn't benefit from its Slipstream trait.)

***Starry Burst (Costs 3 Actions).*** A burst of astral energy flares from the adranach's body. Each creature within 60 feet of it must make a DC 22 Constitution saving throw. On a failed save, a creature takes `dice:5d10|noform|noparens|avg|text(27)` (`5d10`) force damage and is [blinded](rules/5e/conditions.md#Blinded) until the end of its next turn. On a successful save, the creature takes half as much damage and is not [blinded](rules/5e/conditions.md#Blinded). The adranach then turns [invisible](rules/5e/conditions.md#Invisible) until the start of its next turn or until it makes an attack.
```
^statblock