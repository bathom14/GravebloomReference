...
- **Skills** [Athletics](rules/5e/skills.md#Athletics) +11, [Perception](rules/5e/skills.md#Perception) +7
...
***Arcane Leak.*** When the adranach is reduced to half its hit point maximum (85 hit points), its mithral mask cracks and its arcane form begins to waver, creating a field of unstable magic around it. Any creature that starts its turn within 10 feet of the adranach or enters that area for the first time on a turn takes `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) radiant damage. Additionally, if a creature casts a spell within this area, it must make a DC 14 ability check using its spellcasting ability. On a failure, the spell backfires, consuming the spell slot and dealing `dice:1d6|noform|noparens|avg|text(3)` (`d6`) force damage to the caster for each level of the spell slot that was consumed.
...
***Claws.*** *Melee Weapon Attack:* `dice:1d20+11|noform|noparens|text(+11)` to hit, one target, reach 5 ft. *Hit:* `dice:4d12+6|noform|noparens|avg|text(32)` (`4d12 + 6`) slashing damage.

***Force Bolts (Recharge 5-6).*** The adranach targets up to four creatures it can see within 60 feet of it. Each target must succeed on a DC 18 Dexterity saving throw or take `dice:8d6|noform|noparens|avg|text(28)` (`8d6`) force damage.
```
^statblock