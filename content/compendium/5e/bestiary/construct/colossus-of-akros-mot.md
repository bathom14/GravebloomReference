...
- **Skills** [Athletics](rules/5e/skills.md#Athletics) +16, [Perception](rules/5e/skills.md#Perception) +7
...
***Crumbling Destruction.*** When the colossus drops to 0 hit points, it crumbles and is destroyed. Any creature on the ground within 30 feet of the crumbling statue must make a DC 22 Dexterity saving throw, taking `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) bludgeoning damage and `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) fire damage on a failed save, or half as much damage on a successful one.
...
***Spear.*** *Melee  or Ranged Weapon Attack:* `dice:1d20+16|noform|noparens|text(+16)` to hit, reach 15 ft., or range 200/600 ft., one target. *Hit:* `dice:4d6+9|noform|noparens|avg|text(23)` (`4d6 + 9`) piercing damage, or `dice:4d8+9|noform|noparens|avg|text(27)` (`4d8 + 9`) piercing damage if used with two hands to make a melee attack. If the colossus makes a ranged attack with this spear, the spear magically returns to its hand after the attack.

***Sword.*** *Melee Weapon Attack:* `dice:1d20+16|noform|noparens|text(+16)` to hit, reach 15 ft., one target. *Hit:* `dice:6d8+9|noform|noparens|avg|text(36)` (`6d8 + 9`) slashing damage.

***Flames of Akros (Recharge 6).*** Magical flames issue from the colossus toward up to three creatures the colossus can see within 90 feet of it. Each target must make a DC 24 Dexterity saving throw, taking `dice:8d8|noform|noparens|avg|text(36)` (`8d8`) fire damage on a failed save, or half as much damage on a successful one. On a failed save, a target also magically catches fire for 1 minute. At the end of each of its turns thereafter, the burning target repeats the saving throw. It takes `dice:4d8|noform|noparens|avg|text(18)` (`4d8`) fire damage on a failed save, and the effect ends on a successful one.
```
^statblock