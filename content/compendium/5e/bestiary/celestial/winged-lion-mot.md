...
***Pounce.*** If the lion moves at least 20 feet straight toward a creature and then hits it with a claw attack on the same turn, that target must succeed on a DC 15 Strength saving throw or be knocked [prone](rules/5e/conditions.md#Prone). If the target is [prone](rules/5e/conditions.md#Prone), the lion can make one bite attack against it as a bonus action.

## Actions

***Bite.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:2d10+5|noform|noparens|avg|text(16)` (`2d10 + 5`) piercing damage.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:2d8+5|noform|noparens|avg|text(14)` (`2d8 + 5`) slashing damage.
```
^statblock