...
- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Survival](rules/5e/skills.md#Survival) +4
...
***Chardalyn Madness.*** The berserker must roll a `dice:d6|noform|noparens|avg` (`d6`) at the start of each of its turns. On a 1, the berserker does nothing on its turn except speak to a nonexistent, evil master whom it has pledged to serve.
...
***Chardalyn Flail.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) bludgeoning damage, and the target must succeed on a DC 13 Constitution saving throw or be [poisoned](rules/5e/conditions.md#Poisoned) until the end of its next turn.

***Chardalyn Javelin.*** *Melee  or Ranged Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft. or range 30/120 ft., one target. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) piercing damage, and the target must succeed on a DC 13 Constitution saving throw or be [poisoned](rules/5e/conditions.md#Poisoned) until the end of its next turn.
```
^statblock