...
***Tentacle.*** *Melee Weapon Attack:* `dice:1d20+9|noform|noparens|text(+9)` to hit, reach 10 ft., one target. *Hit:* `dice:2d6+6|noform|noparens|avg|text(13)` (`2d6 + 6`) bludgeoning damage plus `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) necrotic damage.
...
- **Psychic Torment.** The target takes `dice:2d12|noform|noparens|avg|text(13)` (`2d12`) psychic damage.  
```
^statblock