...
***Death Burst.*** When the bloodfin dies, it explodes in a cloud of toxic blood. Each creature in a 10-foot-radius sphere centered on the exploding bloodfin, including any creature swallowed by the bloodfin, must succeed on a DC 15 Constitution saving throw or take `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage. A creature inside the bloodfin when it explodes falls [prone](rules/5e/conditions.md#Prone) in the space formerly occupied by the bloodfin and is no longer [blinded](rules/5e/conditions.md#Blinded) or [restrained](rules/5e/conditions.md#Restrained) by it.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+9|noform|noparens|text(+9)` to hit (with advantage if the target is a creature missing any hit points), reach 5 ft., one target. *Hit:* `dice:2d8+5|noform|noparens|avg|text(14)` (`2d8 + 5`) piercing damage, and if the target is a creature, it is [grappled](rules/5e/conditions.md#Grappled) (escape DC 15). Until this grapple ends, the target is [restrained](rules/5e/conditions.md#Restrained), and the bloodfin can't bite another target.

***Tail.*** *Melee Weapon Attack:* `dice:1d20+9|noform|noparens|text(+9)` to hit (with advantage if the target is a creature missing any hit points), reach 10 ft., one target. *Hit:* `dice:4d6+5|noform|noparens|avg|text(19)` (`4d6 + 5`) bludgeoning damage.

## Bonus Actions

***Swallow.*** *Melee Weapon Attack:* `dice:1d20+9|noform|noparens|text(+9)` to hit, reach 5 ft., one Medium or smaller creature the bloodfin is grappling. *Hit:* The bloodfin swallows the target. The swallowed creature is no longer [grappled](rules/5e/conditions.md#Grappled) but is [blinded](rules/5e/conditions.md#Blinded) and [restrained](rules/5e/conditions.md#Restrained). It has total cover against attacks and other effects outside the bloodfin, it takes `dice:4d6|noform|noparens|avg|text(14)` (`4d6`) necrotic damage at the start of each of its turns, and the bloodfin regains hit points equal to the necrotic damage dealt. A bloodfin can have only one creature swallowed at a time.

If the bloodfin takes 30 damage or more on a single turn from the swallowed creature, the bloodfin must succeed on a DC 15 Constitution saving throw at the end of that turn or regurgitate the creature, which falls [prone](rules/5e/conditions.md#Prone) in a space within 5 feet of the bloodfin and is no longer [blinded](rules/5e/conditions.md#Blinded) or [restrained](rules/5e/conditions.md#Restrained) by it.
```
^statblock