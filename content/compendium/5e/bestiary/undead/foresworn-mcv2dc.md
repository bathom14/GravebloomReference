...
***Duty-Bound.*** The foresworn is bound to enacting a singular duty it pledged itself to in life. While this duty is incomplete, whenever the foresworn is destroyed, it re-forms at its previous location after `dice:1d6|noform|noparens|avg|text(3)` (`d6`) days, with all its hit points restored.

***Incorporeal Movement.*** The foresworn can move through other creatures and objects as if they were difficult terrain. It takes `dice:1d10|noform|noparens|avg|text(5)` (`d10`) force damage if it ends its turn inside an object.
...
***Spectral Polearm.*** *Melee Weapon Attack:* `dice:1d20+8|noform|noparens|text(+8)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+5|noform|noparens|avg|text(12)` (`2d6 + 5`) force damage.
...
**2/day each:** [command](compendium/5e/spells/command.md), [detect evil and good](compendium/5e/spells/detect-evil-and-good.md)

**1/day:** [wall of force](compendium/5e/spells/wall-of-force.md)
