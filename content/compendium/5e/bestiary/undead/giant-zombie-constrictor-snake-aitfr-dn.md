...
- **Proficiency Bonus** +3
- **Saving Throws** ⏤
- **Skills** [Perception](rules/5e/skills.md#Perception) +5, [Stealth](rules/5e/skills.md#Stealth) +6
...
***Sunlight Vulnerability.*** While in sunlight, the snake has disadvantage on attack rolls and on Wisdom ([Perception](rules/5e/skills.md#Perception)) checks that rely on sight. When the snake begins its turn in sunlight, it immediately suffers 10 radiant damage.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 10 ft., one target. *Hit:* `dice:3d8+4|noform|noparens|avg|text(17)` (`3d8 + 4`) piercing damage, and the target must make a DC 15 Constitution saving throw, taking `dice:5d6|noform|noparens|avg|text(17)` (`5d6`) poison damage on a failed save, or half as much damage on a successful one.

***Constrict.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:3d8+4|noform|noparens|avg|text(17)` (`3d8 + 4`) bludgeoning damage, and the target is [grappled](rules/5e/conditions.md#Grappled) (escape DC 16). Until this grapple ends, the creature is [restrained](rules/5e/conditions.md#Restrained) and the snake cannot constrict another target.
```
^statblock