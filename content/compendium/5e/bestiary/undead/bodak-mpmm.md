...
***Death Gaze.*** When a creature that can see the bodak's eyes starts its turn within 30 feet of the bodak, the bodak can force it to make a DC 13 Constitution saving throw if the bodak isn't [incapacitated](rules/5e/conditions.md#Incapacitated) and can see the creature. If the saving throw fails by 5 or more, the creature is reduced to 0 hit points unless it is immune to the [frightened](rules/5e/conditions.md#Frightened) condition. Otherwise, a creature takes `dice:3d10|noform|noparens|avg|text(16)` (`3d10`) psychic damage on a failed save.
...
***Fist.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+2|noform|noparens|avg|text(4)` (`1d4 + 2`) bludgeoning damage plus `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) necrotic damage.

***Withering Gaze.*** One creature that the bodak can see within 60 feet of it must make a DC 13 Constitution saving throw, taking `dice:4d10|noform|noparens|avg|text(22)` (`4d10`) necrotic damage on a failed save, or half as much damage on a successful one.
