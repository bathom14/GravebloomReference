...
***Venomous Claws.*** *Melee Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+2|noform|noparens|avg|text(4)` (`1d4 + 2`) slashing damage plus `dice:1d4|noform|noparens|avg|text(2)` (`d4`) poison damage, and the target must succeed on a DC 11 Constitution saving throw or be [poisoned](rules/5e/conditions.md#Poisoned) until the end of the target's next turn.
```
^statblock