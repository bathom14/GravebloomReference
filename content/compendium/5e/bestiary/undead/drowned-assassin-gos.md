...
- **Skills** [Intimidation](rules/5e/skills.md#Intimidation) +5, [Stealth](rules/5e/skills.md#Stealth) +5
...
***Hand Crossbow.*** *Ranged Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, range 30/120 ft., one target. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) piercing damage plus `dice:1d6|noform|noparens|avg|text(3)` (`d6`) poison damage, and the target must succeed on a DC 12 Constitution saving throw or contract [bluerot](rules/5e/diseases.md#Bluerot) (see the "Bluerot" in notes).

***Dagger.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) piercing damage plus `dice:2d8|noform|noparens|avg|text(9)` (`2d8`) poison damage, and the target must succeed on a DC 12 Constitution saving throw or contract [bluerot](rules/5e/diseases.md#Bluerot) (see the "Bluerot" in notes).

***Reveal (1/Day).*** The drowned assassin removes its mask, revealing its rotting face. Each creature of the assassin's choice within 30 feet of it that can see the assassin must succeed on a DC 13 Wisdom saving throw or be [frightened](rules/5e/conditions.md#Frightened) until the end of its next turn.
```
^statblock