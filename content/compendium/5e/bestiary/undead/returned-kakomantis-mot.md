...
- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Acrobatics](rules/5e/skills.md#Acrobatics) +5, [Athletics](rules/5e/skills.md#Athletics) +2, [Stealth](rules/5e/skills.md#Stealth) +5
...
***Shortsword.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) piercing damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage.

***Underworld Bolt.*** *Ranged Spell Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, range 120 ft., one creature. *Hit:* `dice:2d8+2|noform|noparens|avg|text(13)` (`2d8 + 2`) necrotic damage, and the target can't regain hit points until the start of the Returned's next turn. If the target is missing any of its hit points, it instead takes `dice:2d12+2|noform|noparens|avg|text(17)` (`2d12 + 2`) necrotic damage.
```
^statblock