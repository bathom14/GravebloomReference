...
***Pseudopod.*** *Melee Weapon Attack:* `dice:1d20+3|noform|noparens|text(+3)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+1|noform|noparens|avg|text(8)` (`2d6 + 1`) acid damage. If the target is a Medium or smaller creature, it has the [grappled](rules/5e/conditions.md#Grappled) condition (escape DC 11). Ability checks made to escape this grapple have disadvantage.
```
^statblock