...
Most serpentfolk have little love for the [Cloaked Serpent](compendium/5e/deities/exandria-the-cloaked-serpent-tdcsr.md), though they pay him homage as the deity of poisons, assassins, and darkness. For the Vos'skyriss serpentfolk, worship is a means to an end. Their legends say that the devout will be granted the ability to devour nightmares, enabling them to gorge themselves on dreams of fear and terror until they grow so large that they can turn on their god and swallow the [Cloaked Serpent](compendium/5e/deities/exandria-the-cloaked-serpent-tdcsr.md) himself. Once begun, this feast is unending—they will devour the remaining gods one by one, and finally, the entirety of the multiverse.
...
**At will:** [animal friendship](compendium/5e/spells/animal-friendship.md), [speak with animals](compendium/5e/spells/speak-with-animals.md) (snakes only)

**2/day each:** [sleep](compendium/5e/spells/sleep.md), [suggestion](compendium/5e/spells/suggestion.md)

**1/day:** [pass without trace](compendium/5e/spells/pass-without-trace.md)
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one creature. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) piercing damage plus `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) poison damage.

***Scimitar.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d6+3|noform|noparens|avg|text(6)` (`1d6 + 3`) slashing damage.

***Constrict.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one creature. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) bludgeoning damage, and the target is [grappled](rules/5e/conditions.md#Grappled) (escape DC 13). Until this [grapple](rules/5e/variant-rules/unarmed-strike-xphb.md) ends, the creature is [restrained](rules/5e/conditions.md#Restrained), and the serpentfolk can't constrict another target.
```
^statblock
