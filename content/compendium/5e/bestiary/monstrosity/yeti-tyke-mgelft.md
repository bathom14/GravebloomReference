...
***Keen Smell.*** The yeti tyke has advantage on Wisdom ([Perception](rules/5e/skills.md#Perception)) checks that rely on smell.

***Snow Camouflage.*** The yeti tyke has advantage on Dexterity ([Stealth](rules/5e/skills.md#Stealth)) checks made to hide in snowy terrain.

## Actions

***Claw.*** *Melee Weapon Attack:* `dice:1d20+2|noform|noparens|text(+2)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+1|noform|noparens|avg|text(3)` (`1d4 + 1`) slashing damage, plus `dice:1d4|noform|noparens|avg|text(2)` (`d4`) cold damage. Claw does both slashing and cold damage.
```
^statblock