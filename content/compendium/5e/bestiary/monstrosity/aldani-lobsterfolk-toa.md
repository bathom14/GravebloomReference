...
***Claw.*** *Melee Weapon Attack:* `dice:1d20+3|noform|noparens|text(+3)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+1|noform|noparens|avg|text(5)` (`1d8 + 1`) slashing damage, and the target is [grappled](rules/5e/conditions.md#Grappled) (escape DC 11). The aldani has two claws, each of which can grapple only one target.
```
^statblock