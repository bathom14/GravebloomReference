...
***Bizarre Physiology.*** The creeper can't take actions except for [Dash](rules/5e/actions.md#Dash), [Disengage](rules/5e/actions.md#Disengage), [Hide](rules/5e/actions.md#Hide), and [Search](rules/5e/actions.md#Search). It can't take bonus actions or reactions.
...
- If there are one or more Humanoids within 10 feet of the hissing creeper, the creeper explodes in a ball of energy that fills a 20-foot-radius sphere centered on itself. This energy spreads around corners. The creeper is destroyed, and every other creature in the explosion's area must make a DC 12 Dexterity saving throw, taking `dice:4d6|noform|noparens|avg|text(14)` (`4d6`) lightning damage on a failed save, or half as much damage on a successful one. Increase this damage by `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) if the creeper is charged (see "Electrical Charge" below). Objects in the area that aren't being worn or carried take `dice:4d6|noform|noparens|avg|text(14)` (`4d6`) lightning damage, or `dice:6d6|noform|noparens|avg|text(21)` (`6d6`) lightning damage if the creeper is charged.  
...
***Fear of Felines.*** The creeper has the [frightened](rules/5e/conditions.md#Frightened) condition whenever it starts its turn within 60 feet of a feline creature it can see. The condition lasts until the start of the creeper's next turn.
```
^statblock