...
```ad-statblock
title: Boss Delour
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/BMT/Boss%20Delour.webp#token)
...
- **Skills** [Deception](rules/5e/skills.md#Deception) +11, [Perception](rules/5e/skills.md#Perception) +10, [Sleight of Hand](rules/5e/skills.md#Sleight%20of%20Hand) +8, [Stealth](rules/5e/skills.md#Stealth) +8
...
***Evasion.*** If Delour is subjected to an effect that allows him to make a Dexterity saving throw to take only half damage, he instead takes no damage if he succeeds on the saving throw and only half damage if he fails, provided he doesn't have the [incapacitated](rules/5e/conditions.md#Incapacitated) condition.
...
***Bite (Rat or Hybrid Form Only).*** *Melee Weapon Attack:* `dice:1d20+8|noform|noparens|text(+8)` to hit, reach 5 ft., one target. *Hit:* `dice:5d8+4|noform|noparens|avg|text(26)` (`5d8 + 4`) piercing damage. If the target is a Humanoid, it must succeed on a DC 14 Constitution saving throw or be cursed with lycanthropy. While cursed in this way, the target retains its alignment, languages, and equipment but otherwise uses the wererat stat block, excluding actions that require equipment the target doesn't have. During any night when there's a full moon in the sky, the target becomes an NPC under the DM's control and remains so until the night ends. A [Remove Curse](compendium/5e/spells/remove-curse.md) spell or similar magic ends this curse.

***Shortsword (Humanoid or Hybrid Form Only).*** *Melee Weapon Attack:* `dice:1d20+8|noform|noparens|text(+8)` to hit, reach 5 ft., one target. *Hit:* `dice:2d6+4|noform|noparens|avg|text(11)` (`2d6 + 4`) piercing damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage.

***Hand Crossbow (Humanoid or Hybrid Form Only).*** *Ranged Weapon Attack:* `dice:1d20+8|noform|noparens|text(+8)` to hit, range 30/120 ft., one target. *Hit:* `dice:2d6+4|noform|noparens|avg|text(11)` (`2d6 + 4`) piercing damage plus `dice:3d6|noform|noparens|avg|text(10)` (`3d6`) poison damage.
