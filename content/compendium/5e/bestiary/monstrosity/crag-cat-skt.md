...
The crag cat knows its territory and often attacks when its prey is asleep, exhausted, or otherwise weakened. Although crag cats are typically solitary, they can be found in family groups of two parents and `dice:1d4|noform|noparens|avg` (`d4`) Small noncombatant cubs in the spring, or in hungry packs in severe winter weather.
...
- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Stealth](rules/5e/skills.md#Stealth) +7, [Perception](rules/5e/skills.md#Perception) +4
...
***Pounce.*** If the cat moves at least 20 feet straight toward a creature then hits it with a claw attack on the same turn, that target must succeed on a DC13 Strength saving throw or be knocked [prone](rules/5e/conditions.md#Prone). If the target is [prone](rules/5e/conditions.md#Prone), the cat can make one bite attack against it as a bonus action.
...
***Bite.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d10+3|noform|noparens|avg|text(8)` (`1d10 + 3`) piercing damage.

***Claw.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+3|noform|noparens|avg|text(7)` (`1d8 + 3`) slashing damage.
```
^statblock