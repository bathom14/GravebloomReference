...
***Hooked Spear.*** *Melee Weapon Attack:* `dice:1d20+2|noform|noparens|text(+2)` to hit, reach 5 ft., one target. *Hit:* `dice:1d6|noform|noparens|avg|text(3)` (`d6`) piercing damage. If the target is Medium or smaller, the derro can choose to deal no damage and knock it [prone](rules/5e/conditions.md#Prone).

***Light Crossbow.*** *Ranged Weapon Attack:* `dice:1d20+4|noform|noparens|text(+4)` to hit, range 80/320 ft., one target. *Hit:* `dice:1d8+2|noform|noparens|avg|text(6)` (`1d8 + 2`) piercing damage.
```
^statblock
