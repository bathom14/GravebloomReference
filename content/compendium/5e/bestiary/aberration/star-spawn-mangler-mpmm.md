...
***Claw.*** *Melee Weapon Attack:* `dice:1d20+7|noform|noparens|text(+7)` to hit, reach 5 ft., one target. *Hit:* `dice:1d8+4|noform|noparens|avg|text(8)` (`1d8 + 4`) slashing damage. If the attack roll has advantage, the target also takes `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) psychic damage.

***Flurry of Claws (Recharge 5-6).*** The mangler makes six Claw attacks. Either before or after these attacks, it can move up to its speed without provoking [opportunity attacks](rules/5e/actions.md#Opportunity%20Attack).

## Bonus Actions

***Shadow Stealth.*** While in dim light or darkness, the mangler takes the [Hide](rules/5e/actions.md#Hide) action.
```
^statblock