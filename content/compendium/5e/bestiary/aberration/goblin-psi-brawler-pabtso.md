...
- **Armor Class** 15 ([studded leather armor](compendium/5e/items/studded-leather-armor-phb.md))
- **Hit Points** 31 (`7d6 + 7`)
- **Speed** 30 ft.
...
***Mental Burst.*** When the goblin dies, its pent-up mental energy explodes in a psychic blast. Each creature within 5 feet of it must succeed on a DC 13 Intelligence saving throw or take `dice:2d4|noform|noparens|avg|text(5)` (`2d4`) psychic damage.

***Mental Fortitude.*** The goblin has advantage on saving throws against effects that would make it have the [charmed](rules/5e/conditions.md#Charmed) or [frightened](rules/5e/conditions.md#Frightened) condition.

## Actions

***Multiattack.*** The goblin makes two Unarmed Strike attacks.

***Unarmed Strike.*** *Melee Weapon Attack:* `dice:1d20+5|noform|noparens|text(+5)` to hit, reach 5 ft., one target. *Hit:* `dice:1d4+3|noform|noparens|avg|text(5)` (`1d4 + 3`) bludgeoning damage plus `dice:1d6|noform|noparens|avg|text(3)` (`d6`) psychic damage.

## Bonus Actions

***Nimble Escape.*** The goblin takes the Disengage or Hide action.

***Telekinetic Shove.*** The goblin targets one creature it can see within 30 feet of itself with a thrust of telekinetic force. The target must succeed on a DC 13 Strength saving throw or have the [prone](rules/5e/conditions.md#Prone) condition.
```
^statblock