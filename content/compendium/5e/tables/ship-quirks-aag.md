...
| 3 | Any creature that removes itself from the ship's [spelljamming helm](compendium/5e/items/spelljamming-helm-aag.md) receives a startling but harmless magical shock. |
...
| 9 | Unattended tools often go missing, only to reappear `dice:1d4\|noform\|noparens\|avg` (`d4`) hours later in another part of the ship. |
| 10 | A creature seated in the ship's [spelljamming helm](compendium/5e/items/spelljamming-helm-aag.md) hears faint spacefaring chanteys in its mind except when the ship is under attack. |
^ship-quirks