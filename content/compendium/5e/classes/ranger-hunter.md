...
You gain one of the following feature options of your choice. Whenever you finish a [Short](rules/5e/variant-rules/short-rest-xphb.md) or [Long Rest](rules/5e/variant-rules/long-rest-xphb.md), you can replace the chosen option with the other one.

#### Colossus Slayer

Your tenacity can wear down even the most resilient foes. When you hit a creature with a weapon, the weapon deals an extra `dice:1d8|noform|noparens|avg` (`d8`) damage to the target if it's missing any of its [Hit Points](rules/5e/variant-rules/hit-points-xphb.md). You can deal this extra damage only once per turn.
...
[Opportunity Attacks](rules/5e/actions.md#Opportunity%20Attack) have [Disadvantage](rules/5e/variant-rules/disadvantage-xphb.md) against you.

#### Multiattack Defense

When a creature hits you with an attack roll, that creature has [Disadvantage](rules/5e/variant-rules/disadvantage-xphb.md) on all other attack rolls against you this turn.
...
When you take damage, you can take a [Reaction](rules/5e/variant-rules/reaction-xphb.md) to give yourself [Resistance](rules/5e/variant-rules/resistance-xphb.md) to that damage and any other damage of the same type until the end of the current turn.