...
---
# Powered Armor
*Heavy armor ([plate armor](compendium/5e/items/plate-armor-phb.md)), legendary (requires attunement)*  
...
The armor has further capabilities that can be powered either by [energy cells](compendium/5e/items/energy-cell-dmg.md) or by your own life energy. You can use a bonus action to draw power from an energy cell or sacrifice hit points to gain one of the following benefits:
...
- Fire arm-mounted laser: *Ranged Weapon Attack:* `dice:1d20+8|noform|noparens|text(+8)` to hit, range 120 feet, one target. *Hit:* `dice:2d6|noform|noparens|avg` (`2d6`) radiant damage (1 charge or 5 hit points).  
...
- Gain [darkvision](rules/5e/senses.md#Darkvision) to a range of 60 feet for up to 1 hour (1 charge or 5 hit points).  
...
> 
> **Automatic Defenses.** Unless Kwalish deactivates the suit's automatic defenses, no one can approach the armor without setting those defenses off. Treat the armor as a [shield guardian](compendium/5e/bestiary/construct/shield-guardian.md) that has stored a [magic missile](compendium/5e/spells/magic-missile.md) spell cast using a 4th-level spell slot. When the armor is reduced to 0 hit points, its defenses are rendered inert and it can be safely approached.
> 
> **Battle of Wills.** When donned by a new user, the armor deems itself superior and attempts to take possession of that user. The user must succeed on a DC 13 Charisma saving throw or be possessed by the armor. While possessed, the user is [incapacitated](rules/5e/conditions.md#Incapacitated) and loses control of its body but retains its awareness. The armor uses the possessed user's statistics (as adjusted by the armor), but doesn't gain access to the user's knowledge, features, or proficiencies.
> 
