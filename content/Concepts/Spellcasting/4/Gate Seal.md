---
title: Gate Seal
note_type: spell
level: 4
school: Abjuration
classes: []
casting_time: 1 minute
range: 60 feet
components: V, S, M — a broken portal key, which the spell consumes; consumed
duration: 24 hour
ritual: false
concentration: false
source: SatO p.12
---
*Abjuration spell level 4*

You fortify the fabric of the planes in a 30-foot cube you can see within range. Within that area, portals close and can't be opened for the duration. Spells and other effects that allow planar travel or open portals, such as {@spell gate} or {@spell plane shift}, fail if used to enter or leave the area. The cube is stationary.

**At Higher Levels.** **At Higher Levels.** When you cast this spell using a spell slot of 6th level or higher, the spell lasts until dispelled.
